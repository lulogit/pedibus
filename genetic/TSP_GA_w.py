...
class TSP_WIN(object):
      def __init__(self, aRoot, aLifeCount = 1000, aWidth = 560, aHeight = 330):
            self.root = aRoot
      ...
      def DFSpro(self, root, tree):
          order = distSort(self.costs[root])
          newRoot = root
          for i in order:
            if tree[i[0]][0] == -1:
                    if (i[1] + tree[root][1]) < (self.costs[i[0]][0] * self.alpha): # unlabeled and feasible, get next node
                        tree[i[0]][0] = root
                        tree[i[0]][1] = i[1] + tree[root][1]
                        newRoot = i[0]
                        break
                    else : # unlabeled an unfeasible, ignore this node
                        continue
            else:          # labeled
                    continue
          if newRoot != root:
                    self.DFSpro(newRoot,tree)
          return tree
      ...
      def new(self, evt = None):
            ...
            tree[0] = [0,0]
            order = distSort(self.costs[0])
            for i in order:
                if tree[i[0]][0] == -1:
                        tree[i[0]][0] = 0
                        tree[i[0]][1] = i[1] + tree[0][1]
                        tree = self.DFSpro(i[0],tree)
           ...
           #leaves = Leaves(order,self.nNodes)
            print order
            self.line(order)
            self.ga = GA(order, aCosts = self.costs, aAlpha = self.alpha, aCrossRate = 0.8,
                  aMutationRage = 0.6,
